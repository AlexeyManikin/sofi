...
class LLMParser(object):
    ...
    def get_summary_row(self, days_count: int) -> dict:
        cursor = self.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute(
            """SELECT sum(summ) as summ, sum(prompt_tokens) as prompt_tokens, sum(total_tokens) as total_tokens, 
               sum(completion_tokens) as completion_tokens, sum(elapsed_time) as elapsed_time
               FROM cost_structure WHERE type = 'del' AND date > DATE_ADD(NOW(), INTERVAL -%s DAY)""", (days_count,))
        data = cursor.fetchone()
        return {
            "summ": data['summ'],
